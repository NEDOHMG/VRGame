...
public class ObjectSpeed : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        // if (PlayerSkiController.sharedInstance.startGame == true || PlayerSkiController.sharedInstance.resetPositionUser == true)
        if (PlayerSkiController.sharedInstance.startGame == true)
        {
            rb.isKinematic = false; // Check if we are in the game
            PlayerSkiController.sharedInstance.startGame = false;
        }
        // We are in the game
        InTheGame();
    }
